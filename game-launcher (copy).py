...
def launch_wine_game(game):
    ...
    wine = subprocess.Popen(['wine', game.target], cwd=game.working_dir, env=wine_env)
#    wine = subprocess.Popen(['wine', game.target], cwd=game.target.rpartition('/')[0], env=wine_env)
    wine.wait()
...
def launch_game(id):
    ...
    if game_data.resolution is not None:
        ...
        # use_alternate = False
        # if game_data.resolution.endswith('(alt)'):
        #     use_alternate = True
        #     resolution = game_data.resolution[:-6]
        # if use_alternate:
        #     subprocess.call(['/usr/bin/xrandr', '-s', game_data.resolution])
        # else:
        #     subprocess.call(['/usr/bin/nvidia-settings', '--assign',
        #                  'CurrentMetaMode="DFP-0: ' + game_data.resolution + ' { ViewPortIn=' + game_data.resolution + ', ViewPortOut=' + game_data.resolution + ' }"'])
